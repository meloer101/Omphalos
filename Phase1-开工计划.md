...
- [x] 粘贴 20 条杂乱反馈 → 捕获产出去重聚类提议 → 审批 tab 5 分钟内批完 → 落图，全程键盘零录入
- [x] 故意投喂垃圾输入：提议可整体拒绝，图无污染

**验收过程中发现并修复的真实 bug（非计划内条目，但直接堵塞验收）**：粘贴 20 条反馈首次跑全链路时，捕获 job 100% 失败（含 pg-boss 重试）。根因：反馈原文中出现引号（如"建议加一个'最近使用'的快捷入口"）时，`deepseek-v4-pro` 生成 tool-call 参数的 JSON 里没有正确转义内部引号，导致 JSON 语法损坏；`ai` 包内置的 `JSON.parse` 解析失败后把原始字符串原样透传（`invalid:true` 兜底路径），`runtime.ts` 里 `call.input` 因而是字符串而非对象，zod 直接报 `expected object, received string`，整批 20 条全军覆没。这是真实反馈场景一定会撞见的坑，不是测试数据凑巧。**修复**：`lib/agents/runtime.ts` 引入 `jsonrepair` 库，在 zod 校验前，若 `call.input` 仍是原始字符串就先尝试修复语法错误再解析校验一次（`tryRepairJson`，回归测试见 `lib/agents/__tests__/runtime.test.ts`）——不改变"tool-calling + zod"的既定架构（决策 D），只是给这个已知的模型输出故障模式加一层韧性。修复后重跑：20 条反馈成功聚类为 12 条 `proposed` 证据节点，整批接受后图内恰好 12 条 `confirmed`、0 条边；另投喂两轮垃圾输入——纯乱码被模型自己判空（0 提议），模糊无信息量的"仿反馈"文本生成 4 条低质提议，整批拒绝后 `audit_log` 记录 `proposed:16 confirmed:12 rejected:4`，图内无残留节点/边。

**验收方式说明**：受当前环境限制（3000 端口被另一会话的 dev server 占用，Next.js 阻止同目录起第二个 dev 实例），未能用浏览器点击走完整审批 UI；改为直接调用 `/api/capture/text` 走真实异步 job 管线、查询 Postgres/pg-boss 落库状态验证捕获与去重，并在**用户明确授权**下用与 `acceptBatch`/`rejectBatch` Server Action 相同的底层函数（`confirmNode`/`deleteNode`）各模拟一次整批操作，验证审批语义（批量确认/批量拒绝、图无污染）。UI 层的表单绑定、按钮渲染此前已在 1.3/1.4 交付并有测试覆盖，本次未重新验证。
